enrichDoublePoissonExpChances <- function(matches, minMatchdays) {
    ...
    matches$homeChancesGoalsRate <- NA
    matches$visitorsChancesGoalsRate <- NA
    ...
    for(sea in allSeasons) {
        ...
        for(mday in allMatchdays) {
            ...
            for(i in seq(1:nrow(matchdayMatches))) {
                ...
                # Calculate Chances - Goals - Rate
                homeChancesGoalsRate <- calculateChancesGoalsRate(aktMatch, seasonMatches, TRUE)
                matches[matches$matchId == aktMatch$matchId, 'homeChancesGoalsRate'] <-
                    homeChancesGoalsRate
                visitorsChancesGoalsRate <- calculateChancesGoalsRate(aktMatch, seasonMatches, FALSE)
                matches[matches$matchId == aktMatch$matchId, 'visitorsChancesGoalsRate'] <-
                    visitorsChancesGoalsRate
            }
        }
    }
    return(matches)
...
calculateChancesGoalsRate <- function(aktMatch, seasonMatches, home) {
    if(home) {
        team <- aktMatch$homeTeamId
    } else {
        team <- aktMatch$visitorsTeamId
    }
    
    homeMatches <- seasonMatches[seasonMatches$matchday < aktMatch$matchday &
                                    seasonMatches$homeTeamId == team, ]
    visitorsMatches <- seasonMatches[seasonMatches$matchday < aktMatch$matchday &
                                          seasonMatches$visitorsTeamId == team, ]
    
    sumChances <- sum(homeMatches$homeChances) + sum(visitorsMatches$visitorsChances)
    sumGoals <- sum(homeMatches$goalsHome) + sum(visitorsMatches$goalsVisitors)
    
    if(sumChances == 0) {
        return(0)
    }
    return(sumGoals / sumChances)
}
