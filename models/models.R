tuneModelWrapped <- function(modelInput, odds, resultFormula, trControl, idMapping, 
                             tuneGrid, preProcess = NULL, algo = 'xgboost', seed = 16450) {
  saveRDS(odds, file = "C:/RStudioWorkspace/BetLab/data/tmp/odds.Rds")
  saveRDS(idMapping, file = 'C:/RStudioWorkspace/BetLab/data/tmp/idMapping.Rds')
  ...
  if(algo == 'xgboost') {
    ...
    model <- train(form = resultFormula, data = modelInput, method = 'xgbTree',
                   trControl = trControl, tuneGrid = tuneGrid, metric = 'GainPerc',
...
# - idMap is an rownum <-> matchId mapping table and has to be present to match the odds
# - Variable 'odds' has to be present in the environment
betMetricsSummary <- function(data, lev, model, probRatioToBet = 1.1, stake = 1) {
  matchIdVector <- c()
  idMapping <- readRDS(file = 'C:/RStudioWorkspace/BetLab/data/tmp/idMapping.Rds')
  odds <- readRDS(file = 'C:/RStudioWorkspace/BetLab/data/tmp/odds.Rds')
  for(dataRow in rownames(data)) {
    matchIdVector <- c(matchIdVector, idMapping$matchId[as.integer(dataRow)])
  }
  
  metricData<- cbind('matchId' = matchIdVector, data)
